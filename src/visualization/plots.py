...
def generar_plot_5_ventanas_w(ds, vf, resultados, ruta_salida, titulo_evento):
    ...
    for ax, res in zip(axes, resultados):
        ...
        # Línea fina y limpia, sin fondo amarillo
        ax.plot(tiempos_num, res['iso'], color='black', linewidth=1.0, label='Isoterma 0°C (Kalman)')
        
        ax.set_title(f"Configuración: {res['nombre']}", fontsize=13, fontweight='bold', loc='left')
        ax.set_ylabel("Altitud [msnm]")
        ax.set_ylim(500, 5000) # Zoom ajustado a la zona de interés
